...
export class HomePageComponent implements OnInit, AfterViewInit {
  ...
  filterMentors(): void {
    const filteredData = this.mentorList.filter(mentor => {
      ...
      const normalizedFirstName = removeAccents(mentor.first_name.trim().toLowerCase());
      const normalizedLastName = removeAccents(mentor.last_name.trim().toLowerCase());
  ...
  sortMentor(sort: Sort) {
    ...
    this.sortedMentor = data.sort((first, second) => {
      ...
      switch (sort.active) {
        case 'id':
          return this.compareNumber(parseInt(first._id), parseInt(second._id), isAsc);
        case 'email':
        case 'user_status':
          return this.compareString(first[sort.active], second[sort.active], isAsc);
        case 'name':
          return this.compareString(first.last_name, second.last_name, isAsc);
        case 'company':
          return this.compareString(first.company.name, second.company.name, isAsc);
        case 'count_document':
          return this.compareNumber(first[sort.active], second[sort.active], isAsc);
        default:
          return 0;
      }
    });
  ...
  compareString(first: string, second: string, isAsc: boolean) {
    return (first.toLowerCase() < second.toLowerCase() ? -1 : 1) * (isAsc ? 1 : -1);
  }
  ...
  goToMentorDetail(mentorId: string) {
    this.router.navigate(['mentor-detail', mentorId])
  }
  ...
  deleteMentor(mentorId: string) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          'Cancelled',
          'Delete data has been cancelled.',
          'error'
        )
      }
    })
